...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 11 12:13:49 2021
...
def plot_samples_mod(fig, gs, x, nplots, cols):
    x = x.to('cpu')
    
    inner = gridspec.GridSpecFromSubplotSpec(nplots//cols, cols,
                    subplot_spec=gs)
...
def plot_2d_latents_mod(fig, gs, qz, z, y):
    z = z.to('cpu')
    y = y.to('cpu')
...
def plot_interpolations_mod(fig, gs, vae, nplots, cols):
    device = next(iter(vae.parameters())).device
    nrow = 10
    nsteps = 10
    prior_params = vae.prior_params.expand(2 * nrow, *vae.prior_params.shape[-1:])
...
def plot_grid_mod(fig, gs, vae, nplots, cols):
    device = next(iter(vae.parameters())).device
    nrow = 10
    xv, yv = torch.meshgrid([torch.linspace(-3, 3, 10), torch.linspace(-3, 3, 10)])
    zs = torch.cat([xv[:, :, None], yv[:, :, None]], -1)
...
def make_vae_plots(vae, x, y, outputs, training_data, validation_data, tmp_img="tmp_vae_out.png", figsize=(18, 18)):
    ...
    # Defines the number of signals you want to plot in each signal plot
    nplots = 10
    # Defines how many signal columns you want in each signal plot
    cols = 2
    
    # Creates the figure and a gridspec, objects that later we can split with subplots
    fig = plt.figure(figsize=figsize)
    outer = gridspec.GridSpec(3, 3)
    
    # plot the observation
    # axes[0, 0].set_title(r'Observation $\mathbf{x}$')
    plot_samples_mod(fig, outer[0], x, nplots, cols)
    ...
    try:
        ...
        if z.shape[1] == 2:
            ...
            qz = outputs['qz']
            plot_2d_latents_mod(fig, outer[1], qz, z, y)
        else:
            # axes[0, 1].set_title(r'Latent Samples $\mathbf{z} \sim q_\phi(\mathbf{z} | \mathbf{x})$ (t-SNE)')
            # plot_latents(axes[0, 1], z, y)
            print('hehe')
    except Exception as e:
        print("Could not generate the plot of the latent sanples because of exception")
        print(e)
    ...
    plot_samples_mod(fig, outer[2], x_sample, nplots, cols)
    ...
    x_samples = px.sample()
    plot_samples_mod(fig, outer[6], x_samples, nplots, cols)
    ...
    plot_interpolations_mod(fig, outer[7], vae, nplots, cols)
    ...
    if vae.latent_features == 2:
        ...
        px = vae.sample_from_prior(batch_size=x.size(0))['px']
        x_samples = px.sample()
        plot_grid_mod(fig, outer[8], vae, nplots, cols)
